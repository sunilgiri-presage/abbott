...
from app.models import DisplacementStatTimeMaster, DisplacementStatTimeOptimized, MigrationProgressDis
...
def get_last_processed_id():
    ...
    progress = MigrationProgressDis.objects.first()
...
def update_last_processed_id(last_id):
    ...
    MigrationProgressDis.objects.update_or_create(id=1, defaults={"last_processed_id": last_id})
...
def migrate_velocity_data():
    ...
    while True:
        queryset = DisplacementStatTimeMaster.objects.filter(
        ...
        with transaction.atomic():
            DisplacementStatTimeOptimized.objects.bulk_create(batch_data, batch_size=BATCH_SIZE)
