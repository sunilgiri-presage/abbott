...
from app.models import AccelerationStatTimeMaster, AccelerationStatTimeOptimized, MigrationProgressAcc
...
def get_last_processed_id():
    ...
    progress = MigrationProgressAcc.objects.first()
...
def update_last_processed_id(last_id):
    ...
    MigrationProgressAcc.objects.update_or_create(id=1, defaults={"last_processed_id": last_id})
...
def migrate_velocity_data():
    ...
    while True:
        queryset = AccelerationStatTimeMaster.objects.filter(
        ...
        with transaction.atomic():
            AccelerationStatTimeOptimized.objects.bulk_create(batch_data, batch_size=BATCH_SIZE)
